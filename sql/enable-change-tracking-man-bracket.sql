...
-- 2) Enable Change Tracking for TB_R_MAN_BRACKET table if not enabled
IF NOT EXISTS (
    SELECT 1
...
BEGIN
    ...
    PRINT 'Table Change Tracking enabled for TB_R_MAN_BRACKET.'
END
ELSE
BEGIN
    PRINT 'Table Change Tracking already enabled for TB_R_MAN_BRACKET.'
END
GO
...
WHERE ctt.object_id = OBJECT_ID(N'dbo.TB_R_MAN_BRACKET')
GO
